...
tyr.roar = () => "RAWERSRARARWERSARARARRRR!";
...
const universities = graduates.map((grad) => {return grad.university});
console.log(universities);
const sorted = universities.sort((a,b) => (a > b) ? 1: -1);
console.log(sorted);
...
const contactInfo = graduates.map((grad) => {return `${grad.first_name} ${grad.email}`});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = []; 
graduates.forEach((grad) => { if (grad.university.includes('Uni')){ unisWithUni.push(grad.university); } });
